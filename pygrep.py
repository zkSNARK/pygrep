...
"""Utility script to mimic grep in python.
    ...
    pygrep.py [-i | --input] <filename> <search str1> <search str2> ... -o <outfile>

    The -o is optional.

    Example use1 : This example will search through the file pygrep.py for the
                   strings '4e0000', 'Poll', and 'cancel'.  If all 3 are found
                   in a given line, it will output the line to stdout.

        python3 pygrep.py 4e0000 Poll cancel -o blah.txt

    Example use 2: This example will search through the file pygrep.py for the
                   strings '4e0000', 'Poll', and 'cancel'.  If all three are
                   found in a given line, it will output the line to both stdout
                   and to the file blah.txt.
...
def grep(infile: str, filter_by: List[str], outfile=None):
    ...
    filters = ', '.join('"{0}"'.format(w) for w in filter_by)
    ...
    try:
        ...
        print("    usage : <infile> <search str1> <search str2> ...", file=sys.stderr)
